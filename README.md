...
> **☁️ RunPod deployment:** For a one-shot, headless GPU batch job (download `.mp4`s from S3 → run inference → write annotated videos, preview images, and CSV summaries to `/workspace/output` → exit), see **[RUNPOD.md](RUNPOD.md)**. Entry points: `infer.py` (script) and `run.sh` (runner); config via env vars in [`.env.example`](.env.example).
